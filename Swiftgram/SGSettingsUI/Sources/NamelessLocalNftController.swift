...
private func namelessLocalNftEntries(state: NamelessLocalNftState) -> [NamelessLocalNftEntry] {
    ...
    id += 1
    entries.append(NamelessLocalNftEntry.action(id: id, section: NamelessLocalNftSection.giftEditor, text: "Добавить NFT-подарок", kind: ItemListActionItemKind.generic, action: NamelessLocalNftEntry.Action.addGift))
    id += 1
    ...
    id += 1
    entries.append(NamelessLocalNftEntry.action(id: id, section: NamelessLocalNftSection.usernameEditor, text: "Добавить NFT-username", kind: ItemListActionItemKind.generic, action: NamelessLocalNftEntry.Action.addUsername))
    id += 1
    ...
    if settings.localNftGifts.isEmpty && settings.localNftUsernames.isEmpty {
        entries.append(.info(id: id, section: .current, text: "Пока нет локальных NFT. Добавленные элементы будут отображаться только в [...]))
        id += 1
    } else {
        for gift in settings.localNftGifts {
            let owner = gift.peerId.isEmpty ? "self" : gift.peerId
            entries.append(NamelessLocalNftEntry.action(id: id, section: NamelessLocalNftSection.current, text: "\(gift.emoji) \(gift.title) \(gift.serial) · \(gift.rarity) · \(owner)", kind: ItemListActionItemKind.destructive, action: NamelessLocalNftEntry.Action.removeGift(gift.id)))
            id += 1
        }
        for username in settings.localNftUsernames {
            let owner = username.peerId.isEmpty ? "self" : username.peerId
            entries.append(NamelessLocalNftEntry.action(id: id, section: NamelessLocalNftSection.current, text: "@\(username.username) \(username.number) · \(owner)", kind: ItemListActionItemKind.destructive, action: NamelessLocalNftEntry.Action.removeUsername(username.id)))
            id += 1
        }
    }
...
public func namelessLocalNftController(context: AccountContext, onSave: @escaping () -> Void) -> ViewController {
    ...
    let arguments = NamelessLocalNftArguments(
        ...
        perform: { action in
            ...
            switch action {
            case .addGift:
                settings.addLocalNftGift(NamelessLocalNftGift(peerId: peerId, title: trimmed(state.giftTitle), subtitle: trimmed(state.giftSubtitle), emoji: trimmed(state.giftEmoji).isEmpty ? "�[...]", rarity: trimmed(state.giftRarity), serial: trimmed(state.giftSerial)))
            case .addUsername:
