...
Его же урезанная версия вшита в клиент как встроенный плагин
(`Swiftgram/MGPluginKit/Sources/MGBuiltinPlugins.swift`) — она всегда включена и в списке не
показывается.

Строки, которые плагин добавляет через `mg.addSettingsRow`, появляются внизу экрана Плагинов
в разделе **НАСТРОЙКИ ПЛАГИНОВ**.
