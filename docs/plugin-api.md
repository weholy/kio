...
`docs/plugins/megram-notifications.plugin` — рабочий плагин уведомлений с настройками,
фильтрами, дедупликацией и статистикой. Скиньте файл себе в «Избранное» и нажмите на него,
либо установите через **Плагины → «+»**.

Встроенных плагинов сейчас нет: экран Плагинов — это инвентарь (название, автор, описание,
тумблер), а не панель управления. Механизм остался — см.
`Swiftgram/MGPluginKit/Sources/MGBuiltinPlugins.swift`.

`mg.addSettingsRow` принимается и хранится, но своего экрана у строк плагина пока нет —
используйте `mg.alert` / `mg.toast` для взаимодействия с пользователем.
