    ...
    func updateLayout(widthClass: ContainerViewLayoutSizeClass, constrainedWidth: CGFloat, constrainedHeight: CGFloat, minimalWidth: CGFloat?, transition: ContainedViewLayoutTransition) -> CGSize {
        ...
        case .regular:
            if self.effectView == nil && self.liquidGlassView == nil {
                // Official Apple liquid glass instead of UIBlurEffect
                let isDark = self.presentationData.theme.rootController.keyboardColor == .dark
                let effectView = makeOfficialLiquidGlassEffectView(isDark: isDark, interactive: true)
