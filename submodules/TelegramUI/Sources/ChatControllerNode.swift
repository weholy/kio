...
private final class NamelessVideoBackgroundView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.dimView.backgroundColor = UIColor(white: 0.0, alpha: 0.28)
    ...
    func update(enabled: Bool, path: String) {
        guard enabled, !path.isEmpty, FileManager.default.fileExists(atPath: path) else {
...
class ChatControllerNode: ASDisplayNode, ASScrollViewDelegate {
    ...
    private let namelessVideoBackgroundView: NamelessVideoBackgroundView
    private var namelessVideoBackgroundObserver: NSObjectProtocol?
    ...
    init(context: AccountContext, chatLocation: ChatLocation, chatLocationContextHolder: Atomic<ChatLocationContextHolder?>, subject: ChatControllerSubject?, controllerInteraction: ChatControllerInteraction, chatPresentationInterfaceState: ChatPresentationInterfaceState, automaticMediaDownloadSettings: MediaAutoDownloadSettings, navigationBar: NavigationBar?, statusBar: StatusBar?, backgroundNode: WallpaperBackgroundNode, controller: ChatControllerImpl?) {
        ...
        self.namelessVideoBackgroundView = NamelessVideoBackgroundView()
        ...
        self.contentContainerNode.view.insertSubview(self.namelessVideoBackgroundView, at: 0)
        ...
        self.updateNamelessVideoBackground()
        self.namelessVideoBackgroundObserver = NotificationCenter.default.addObserver(forName: .namelessVideoBackgroundDidChange, object: nil, queue: .main) { [weak self] _ in
            self?.updateNamelessVideoBackground()
        }
    ...
    deinit {
        ...
        if let observer = self.namelessVideoBackgroundObserver {
    ...
    private func updateNamelessVideoBackground() {
        let settings = SGSimpleSettings.shared
        let enabled = settings.namelessVideoBackgroundEnabled && !settings.namelessVideoBackgroundPath.isEmpty
        self.namelessVideoBackgroundView.update(enabled: enabled, path: settings.namelessVideoBackgroundPath)
        self.backgroundNode.alpha = enabled ? 0.0 : 1.0
    }
    ...
    func containerLayoutUpdated(_ layout: ContainerViewLayout, navigationBarHeight: CGFloat, transition protoTransition: ContainedViewLayoutTransition, listViewTransaction: (ListViewUpdateSizeAndInsets, CGFloat, Bool, @escaping () -> Void) -> Void, updateExtraNavigationBarBackgroundHeight: (CGFloat, CGFloat, CGSize?, ContainedViewLayoutTransition) -> Void) {
        ...
        transition.updateFrame(view: self.namelessVideoBackgroundView, frame: CGRect(origin: CGPoint(), size: layout.size))
        self.updateNamelessVideoBackground()
