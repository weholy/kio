            ...
            return (layout, { [weak self] in
                if let strongSelf = self {
                    ...
                    if let glassOverlay = strongSelf.backgroundNode.sgGlassOverlay {
                        ...
                        let _glassHasCorners = itemListHasRoundedBlockLayout(params)
                        var _glassTop = false
                        var _glassBottom = false
                        if case .sameSection(false) = neighbors.top {} else { _glassTop = true }
                        if case .sameSection(false) = neighbors.bottom {} else { _glassBottom = true }
                        let _glassR = NamelessItemListGlass.sectionCornerRadius
                        glassOverlay.updateLayout(size: bgFrame.size, topLeft: (_glassHasCorners && _glassTop) ? _glassR : 0, topRight: (_glassHasCorners && _glassTop) ? _glassR : 0, bottomLeft: (_glassHasCorners && _glassBottom) ? _glassR : 0, bottomRight: (_glassHasCorners && _glassBottom) ? _glassR : 0, isDark: item.presentationData.theme.overallDarkAppearance)
                    }
                    ...
                    strongSelf.maskNode.image = SGLiquidGlassZone.settings.isEnabled ? nil : (hasCorners ? PresentationResourcesItemList.cornersImage(item.presentationData.theme, top: hasTopCorners, bottom: hasBottomCorners, glass: item.systemStyle == .glass) : nil)
