...
//        })
        ...
        // Official Apple liquid glass (not UIBlurEffect)
        self.effectView = makeOfficialLiquidGlassEffectView(
            isDark: theme.backgroundType != .light,
            interactive: false
        )
...
//        self.containerNode.addSubnode(self.effectNode)
    ...
    func updateTheme(_ theme: AlertControllerTheme) {
        self.effectView.effect = makeOfficialLiquidGlassEffect(
            isDark: theme.backgroundType != .light,
            interactive: false
        )
        // Transparent host so liquid glass is visible
