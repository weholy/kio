...
class TabBarNode: ASDisplayNode, ASGestureRecognizerDelegate {
    ...
    init(theme: PresentationTheme, itemSelected: @escaping (Int, Bool, [ASDisplayNode]) -> Void, contextAction: @escaping (Int, ContextExtractedContentContainingNode, ContextGesture) -> Void, swipeAction: @escaping (Int, TabBarItemSwipeDirection) -> Void) {
        ...
        // Official Apple liquid glass tab bar (GlassBackgroundView → UIGlassEffect.regular)
        let g = SGLiquidGlassNode()
        g.glassTintColor = .clear
        g.glassKind = .panel
        g.interactive = true
        g.isDark = theme.overallDarkAppearance
    ...
    func updateLayout(size: CGSize, leftInset: CGFloat, rightInset: CGFloat, additionalSideInsets: UIEdgeInsets, bottomInset: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        // Official liquid glass tab bar — hide solid fill so UIGlassEffect shows
        if let g = self.glassNode {
            ...
            g.glassKind = .panel
            g.isDark = self.theme.overallDarkAppearance
            g.glassVisible = glassOn
            self.backgroundNode.alpha = glassOn ? 0.0 : 1.0
            // Floating glass pill (reference Whitegram tab bar)
            g.glassCornerRadii = GlassRadii(radius: glassOn ? 28.0 : 0.0)
            g.refreshGlass(zone: .tabBar)
        }
