...
public enum MegramAppearanceStore {
    ...
    /// The stored switch on its own, ignoring whether a file is present.
    ///
    /// The settings row has to read this rather than `isEnabled`, which folds
    /// the file check in: with no file yet, `isEnabled` reports false no matter
    /// what the user just did, so the switch would silently spring back and the
    /// slot could never be set up in the first place.
    public static func isSwitchedOn(_ slot: Slot) -> Bool {
        return self.defaults.bool(forKey: slot.enabledKey)
    }
